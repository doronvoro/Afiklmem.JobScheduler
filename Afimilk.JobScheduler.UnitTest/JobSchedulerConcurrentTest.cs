...
{
    public class JobSchedulerConcurrentTest
    {
        ...
        public JobSchedulerConcurrentTest()
        {
        ...
        public async Task JobExecutionTimes_ShouldStartAndFinishWithinTimeLimit()
        {
            ...
            Assert.All(startTimes, t => Assert.True((startTime - t.Value).TotalMilliseconds < timeLimit.TotalMilliseconds, "Job start times exceed the time limit."));
            Assert.All(endTimes, t => Assert.True((endTime - t.Value).TotalMilliseconds < timeLimit.TotalMilliseconds, "Job end times exceed the time limit."));

            var maxGap = jobExecution.Max(job => (job.ExecutionCompleted.Value - job.ExecutionStarted.Value).TotalMilliseconds);
